...
try {
    ...
    foreach ($OldReleaseFile in @("crc-lnm-medical-agent-twomeme-17-$Version-source.zip", "RELEASE_CHECKSUMS.sha256")) {
    ...
    $env:CANARY_CONSOLE = Join-Path $Venv "Scripts/crc-lnm-medical-agent-twomeme-17.exe"
    ...
    $SourceZip = Join-Path $Root "crc-lnm-medical-agent-twomeme-17-$Version-source.zip"
